...
export class CrazyKey {
    ...
    private static genCrazyKey(isSuffix: boolean): string {
        ...
        const random = this.asciiGuru.find(i => i.key === alpha.toUpperCase()).val;
        const key = (isSuffix ? '' : alpha) + this.genRandomChar(random) + (isSuffix ? alpha : '');
        return key;
    }

    private static genRandomChar(length): string {
        let result = '';
    ...
    public static decode(key: string): string {
        try {
            ...
            const preNum = this.asciiGuru.find(i => i.key === preAlpha.toUpperCase()).val + 1;

            const sufAlpha = key.charAt(key.length - 1);
            const sufNum = this.asciiGuru.find(i => i.key === sufAlpha.toUpperCase()).val + 1;
